                    ...
                    <button data-id="{{ $project->id }}"
                        ...
                        @php
                            ...
                            foreach($project->applicants as $applicant) {
                                array_push($applicantArr, $applicant->user_id);
                            };
                        @endphp
                        {{in_array(auth()->user()->id, $applicantArr) || $project->created_by == auth()->user()->id || auth()->user()->is_complete == 0 ? 'disabled' : '' }}>I'M IN</button>
                <div
                    class="flex flex-col justify-center items-center custom-bg-green px-2 py-3 rounded-full absolute -top-8 right-8">
                    <span class="text-white text-2xl font-semibold applicantCount">{{ $project->applicants->count() }}</span>
